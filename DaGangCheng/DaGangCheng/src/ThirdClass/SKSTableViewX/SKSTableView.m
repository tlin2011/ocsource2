...
{
    //NSLog(@"sksTable-heightForRow:%@ expandedIndexPaths:%@",indexPath,_expandedIndexPaths);
    if (![self.expandedIndexPaths[indexPath.section] containsObject:indexPath]) {
        return [self.SKSTableViewDelegate tableView:tableView heightForRowAtIndexPath:indexPath];
        
    } else {
       return [self.SKSTableViewDelegate tableView:tableView heightForSubRowAtIndexPath:indexPath];
    }
}
/*
