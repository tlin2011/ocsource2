...
#define WKTableViewCellButtonWidth 80.0f
...
withRightButtonTitles:(NSArray *)rightButtonTitles{
    ...
    if (self) {
        ...
        for (int a=0; a<self.rightButtonTitles.count; a++) {
            ...
            //2014-10-17 fred
            if ([self.rightButtonTitles[a] isEqualToString:@"Delete"]) {
                [button setImage:[UIImage imageNamed:@"删除_社区.png"] forState:UIControlStateNormal];
                [button setImage:[UIImage imageNamed:@"删除_社区_h.png"] forState:UIControlStateHighlighted];
            } else {
                [button setTitle:self.rightButtonTitles[a] forState:UIControlStateNormal]; //old source
            }
            //--end fred
