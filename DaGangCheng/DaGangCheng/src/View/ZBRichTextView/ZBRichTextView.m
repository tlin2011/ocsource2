...
- (void)createDefault
{
    ...
    self.runTypeList = ZBRichTextRunURLType | ZBRichTextRunEmojiType | ZBRichTextRunTelephoneType;
    self.lineSpace   = 2.0f;
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.attributedText == nil)
    {
        ...
        if ([self.delegage respondsToSelector:@selector(richTextView:modifyAttributedText:)]) {
            [self.delegage richTextView:self modifyAttributedText:attString];
        }
    }
    else
    {
    ...
    for (int i = 0; i < CFArrayGetCount(lines); i++)
    {
        ...
        for (int j = 0; j < CFArrayGetCount(runs); j++)
        {
            ...
            ZBRichTextRun *richTextRun = [attributes objectForKey:kZBRichTextRunSelfAttributedName];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    {
        __weak ZBRichTextView *weakSelf = self;
...
+ (NSMutableAttributedString *)createAttributedStringWithText:(NSString *)text font:(UIFont *)font lineSpace:(CGFloat)lineSpace
{
    ...
    CTParagraphStyleRef style = CTParagraphStyleCreate(settings, 2);//sizeof(settings));
...
+ (NSArray *)createTextRunsWithAttString:(NSMutableAttributedString *)attString runTypeList:(ZBRichTextRunTypeList)typeList
{
    ...
    if (typeList != ZBRichTextRunNoneType)
    {
        if ((typeList & ZBRichTextRunURLType) == ZBRichTextRunURLType)
        {
            [array addObjectsFromArray:[ZBRichTextRunURL runsForAttributedString:attString]];
        }
        
        if ((typeList & ZBRichTextRunEmojiType) == ZBRichTextRunEmojiType)
        {
            [array addObjectsFromArray:[ZBRichTextRunEmoji runsForAttributedString:attString]];
        }
        
        if ((typeList & ZBRichTextRunTelephoneType) == ZBRichTextRunTelephoneType)
        {
            [array addObjectsFromArray:[ZBRichTextRunTelephone runsForAttributedString:attString]];
        }
    }
...
{
    [[self class] createTextRunsWithAttString:attString runTypeList:ZBRichTextRunURLType | ZBRichTextRunEmojiType | ZBRichTextRunTelephoneType];
