...
@interface NearHotPindaoTableVC ()
{
    //ApiUrl * url;
...
@implementation NearHotPindaoTableVC
...
- (void)viewDidLoad
{
    ...
    [self.tableView registerClass:[NearHotPindaoTableCell class] forCellReuseIdentifier:@"NearHotPindaoTableCell"];
...
-(void)requestUsers
{
    ...
    [request urlStr:[ApiUrl nearPindaoUrlStr] parameters:parameters passBlock:^(NSDictionary *customDict) {
...
{
    NearHotPindaoTableCell *cell = [tableView dequeueReusableCellWithIdentifier:@"NearHotPindaoTableCell"];
    ...
    NSString *distanceStr = [NSString stringWithFormat:@"%@",[HuaxoUtil getFriendlyDistance:[NSNumber numberWithInteger:[pindao.distance integerValue]]]];
    NSMutableAttributedString *astr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@  %@",distanceStr,pindao.addr]];
    [astr addAttribute:NSForegroundColorAttributeName value:UIColorFromRGB(0x32373e) range:NSMakeRange(0, distanceStr.length)];
    cell.content.attributedText = astr;

    //cell.addr.text = pindao.addr;
    //粉丝 和 今日贴数
