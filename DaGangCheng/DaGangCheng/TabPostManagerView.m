...
@implementation TabPostManagerView
...
- (void)initSubviews {
    ...
    self.view.bounds = CGRectMake(0, 0, 150, self.titles.count * ButtonTitleHeight);
    self.viewFrame = CGRectMake(0, 0, self.view.bounds.size.width, self.view.bounds.size.height);
}
...
- (void)show {
    ...
    UIView *BgView = [[UIView alloc] initWithFrame:self.viewFrame];
...
- (void)clickedButton:(UIButton *)sender {
    if ([self.delegate respondsToSelector:@selector(tabPostManagerView:selectedButtonTitle:)]) {
        [self.delegate tabPostManagerView:self selectedButtonTitle:[sender titleForState:UIControlStateNormal]];
